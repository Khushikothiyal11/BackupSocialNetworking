...
const loginUser=async(req, res, next) =>{
    try{
      ...
      const token = await jwt.sign({id: user?._id}, process.env.JWT_SECRET,{expiresIn:"54h"})
      res.json({token, id: user?._id,fullName:user.fullName, profilePhoto:user?.profilePhoto}).status(200)
...
const getUserPosts = async (req, res, next) => {
    try {
      ...
      const userWithPosts = await UserModel.findById(userId).populate({
        path: "posts",
        options: { sort: { createdAt: -1 } } // sorts newest first
      });
      ...
      return next(new HttpError("Unable to fetch user posts", 500));
    }
  };
...
module.exports = {registerUser, loginUser, getUser, getUsers, getUserPosts, editUser, followUnfollowUser, changeUserAvatar}