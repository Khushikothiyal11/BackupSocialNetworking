...
const friendsModel = require("../models/friendModel");
...
const getPhotoGallries = async (req, res) => {
  try {
    const galleries = await PostModel.find();
    console.log("Fetched galleries:", galleries);
    res.json(galleries).status(200);
  } catch (error) {
    console.error("Error fetching galleries:", error);
    return next(new HttpError("Unable to fetch galleries", 500));
  }
};
